...
async function getMyRequests(userId) {
  return sessionsRepo.getMyRequests(userId);
}
...
async function updateSession(sessionId, userId, data) {
  ...
  if (session.creator_id !== userId) {
    const err = new Error('Accès refusé. Seul le créateur peut modifier la session.'); err.status = 403; throw err;
  }
  const confirmed = await sessionsRepo.getAcceptedRequestCount(sessionId);
  if (confirmed > 0) {
    const err = new Error('Impossible de modifier une session avec des participants confirmés.'); err.status = 409; throw err;
  }
  const preferences = {};
  if (data.level_min != null) preferences.level_min = data.level_min;
  if (data.gender    != null) preferences.gender    = data.gender;
  return sessionsRepo.update(sessionId, {
    date:        data.date,
    time:        data.time,
    end_time:    data.end_time || null,
    max_players: data.max_players,
...
module.exports = { create, list, listMySessions, getMyRequests, getById, updateStatus, updateSession, getHistory, getSessionParticipants };
